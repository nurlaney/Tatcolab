...
@include('backend.forms.lng_selector', ['lngs'=>$lngs])
        <form action="{{route('admin.brands.update',[$brandsL[0]->u_id])}}" enctype="multipart/form-data" file="true" method="post" class="form-horizontal form-label-left">
            @csrf
    ...
    <div class="card">
        <div class="card-body">
         ...
         <div class="col">   
        @foreach($brandsL as $brand) 
        @if($brand->lng) 
            <div class="row lng-form" id="{{ $brand->l_id }}" @if(!$brand->lng->default) style="display: none" @endif> 
                 @include('backend.forms.number', 
                    ['label'=>'Ordering', 
                    'name'=>'ordering', 
                    'lng'=>$brand->lng, 
                    'value'=>$brand->ordering, 
                    'required'=>0]) 
        ...
        <div class="col-md-12 col-sm-12 col-xs-12"> 
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-4 col-xs-12"> 
                    <input type="text" class="form-control" name='tag' value='{{$brandsL[0]->tag}}'/> 
                </div> 
            </div>
            </div> 
        </div>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-4 col-xs-12"> 
                    <select class="select2_group form-control" name="sub_id" id="sub_id"> 
                        ...
                        <option value="{{ $option->u_id }}" @if($brand->sub_id == $option->u_id) selected @endif>{{ $option->title }}</option> 
                        @endforeach 
                    </select> 
                </div> 
            </div>
